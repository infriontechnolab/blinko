...
import { useVendorOrders, type VendorOrder } from "@/lib/store/vendor-orders-store";
...
/** Qty/revenue for a single order, optionally scoped to one product. */
function orderQty(o: VendorOrder, productId: string | "all") {
  const items = productId === "all" ? o.items : o.items.filter((i) => i.productId === productId);
  return items.reduce((sum, i) => sum + i.qty, 0);
}
function orderRevenue(o: VendorOrder, productId: string | "all") {
  if (productId === "all") return o.subtotal;
  return o.items
...
function ReportsContent() {
  ...
  const fromKey = dayKey(range?.from ?? today);
  const toKey = dayKey(range?.to ?? range?.from ?? today);

  const completedOrders = orders.filter(
    (o) => o.vendorStatus !== "rejected" && o.vendorStatus !== "cancelled",
  );
  const inRange = completedOrders.filter((o) => {
    const key = dayKey(new Date(o.placedAt));
    return key >= fromKey && key <= toKey;
  });
  const matching =
    productId === "all" ? inRange : inRange.filter((o) => orderQty(o, productId) > 0);

  const totalRevenue = matching.reduce((sum, o) => sum + orderRevenue(o, productId), 0);
  const totalOrders = matching.length;
  const avgOrderValue = totalOrders > 0 ? totalRevenue / totalOrders : 0;

  const byDay = datesBetween(fromKey, toKey).map((d) => {
    const key = dayKey(d);
    const dayOrders = matching.filter((o) => dayKey(new Date(o.placedAt)) === key);
    return {
      label: d.toLocaleDateString(undefined, { month: "short", day: "numeric" }),
      count: dayOrders.length,
      revenue: dayOrders.reduce((sum, o) => sum + orderRevenue(o, productId), 0),
    };
  });
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 md:p-8">
      ...
      <div className="mt-6 flex flex-col gap-4 lg:flex-row">
        ...
        <div className="flex shrink-0 flex-col gap-3 rounded-2xl border border-border bg-surface p-4 sm:flex-row lg:w-auto lg:flex-col lg:justify-start">
          <Select value={productId} onValueChange={setProductId}>
            <SelectTrigger className="sm:w-56 lg:w-56">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="justify-start font-normal sm:w-56 lg:w-56">
      ...
      <div className="mt-6 rounded-2xl border border-border bg-surface p-5">
        <p className="mb-4 text-sm font-bold">
          Sales — {fromKey === toKey ? fromKey : `${fromKey} – ${toKey}`}
        </p>
